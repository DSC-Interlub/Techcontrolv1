...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (chamadoId && !chamado) {
      const { data } = await supabase.from('chamados').select('*').eq('id', chamadoId).single();
      chamado = data;
    }
    ...
    Promise.all([
      send(chamado.solicitante_email, `[TechControl] Chamado ${chamado.numero_chamado} aberto com sucesso`, htmlUsuario(chamado)),
      send(ADM_EMAIL, `[TechControl] Novo chamado: ${chamado.numero_chamado} — ${chamado.solicitante_nome}`, htmlAdmin(chamado))

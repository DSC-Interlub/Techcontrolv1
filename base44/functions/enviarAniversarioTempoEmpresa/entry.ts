...
Deno.serve(async (req) => {
  ...
  if (demandas.length === 0) {
    console.log(`[enviarAniversarioTempoEmpresa] Nenhuma demanda para hoje (${hojeStr}).`);
  ...
  for (const demanda of demandas) {
    ...
    try {
      ...
      enviados++;
      console.log(`[enviarAniversarioTempoEmpresa] ${colaborador.nome_completo} — ${anos} anos: ${destinatarios.length} destinatários.`);
    } catch (erro) {

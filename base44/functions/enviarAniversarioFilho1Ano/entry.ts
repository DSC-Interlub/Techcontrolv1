...
Deno.serve(async (req) => {
  ...
  if (demandas.length === 0) {
    console.log(`[enviarAniversarioFilho1Ano] Nenhuma demanda para hoje (${hojeStr}).`);
  ...
  for (const demanda of demandas) {
    ...
    try {
      ...
      enviados++;
      console.log(`[enviarAniversarioFilho1Ano] ${colaborador.nome_completo} — ${nomeFilho}: ${destinatarios.length} destinatários.`);
    } catch (erro) {

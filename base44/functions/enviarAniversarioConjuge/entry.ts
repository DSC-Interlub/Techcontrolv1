...
Deno.serve(async (req) => {
  ...
  if (demandas.length === 0) {
    console.log(`[enviarAniversarioConjuge] Nenhuma demanda para hoje (${hojeStr}).`);
  ...
  for (const demanda of demandas) {
    ...
    if (!colaborador || !ehHojeMesDia(colaborador.conjuge_data_nascimento)) continue;
    ...
    try {
      ...
      enviados++;
      console.log(`[enviarAniversarioConjuge] ${colaborador.nome_completo}: ${destinatarios.length} destinatários.`);
    } catch (erro) {
      console.error(`[enviarAniversarioConjuge] Erro para ${colaborador.nome_completo}:`, erro.message);
      await base44.asServiceRole.entities.Comunicados_Log.create({
        tipo_comunicado: 'aniversario_conjuge',
        colaborador_nome: colaborador.nome_completo,
        colaborador_id: colaborador.id,

...
import { Plus, Pencil, Trash2, Pen, Search, Users, List, UserPlus, UserMinus, X } from "lucide-react";
...
export default function Canetas_Vibracao() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['canetas_vibracao'] });
      setShowForm(false);
      setFormData({});
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => base44.entities.Canetas_Vibracao.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['canetas_vibracao'] });
      setShowForm(false);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => base44.entities.Canetas_Vibracao.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['canetas_vibracao'] });
    },
  });
  ...
  const allUsers = Array.from(new Set([
    ...
    ...coletores.filter(e => e.usuario_atual).map(e => e.usuario_atual),
    ...
    <div className="p-4 md:p-8 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-pink-100 rounded-xl flex items-center justify-center">
              <Pen className="w-6 h-6 text-pink-600" />
            </div>
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Canetas de Vibração</h1>
              <p className="text-gray-500 mt-1">Gerenciar canetas de vibração</p>
            </div>
          </div>
          <Button
            onClick={() => {
          ...
          <Card className="mb-6">
            <CardHeader className="border-b">
              <div className="flex items-center justify-between">
                <CardTitle>{editingEquipamento ? "Editar Caneta" : "Nova Caneta"}</CardTitle>
            ...
            <form onSubmit={handleSubmit}>
              <CardContent className="pt-6 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <Input placeholder="Ex: CAN001" value={formData.numero_sequencial || ""} onChange={(e) => setFormData({ ...formData, numero_sequencial: e.target.value })} />
                  </div>
                  <div>
                  ...
                  <div>
                    <Label>Tipo</Label>
                    <Input placeholder="Tipo da caneta" value={formData.tipo || ""} onChange={(e) => setFormData({ ...formData, tipo: e.target.value })} />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label>Marca</Label>
                    <Input placeholder="Marca da caneta" value={formData.marca || ""} onChange={(e) => setFormData({ ...formData, marca: e.target.value })} />
                  </div>
                  <div>
                    <Label>Modelo</Label>
                    <Input placeholder="Modelo da caneta" value={formData.modelo || ""} onChange={(e) => setFormData({ ...formData, modelo: e.target.value })} />
                  </div>
                </div>
              ...
              <CardFooter className="border-t pt-6 flex justify-end gap-3">
                ...
                <Button type="submit" className="bg-pink-600 hover:bg-pink-700" disabled={createMutation.isPending || updateMutation.isPending}>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="relative w-full md:w-64">
                  ...
                  <Input
                    placeholder={viewMode === "grouped" ? "Buscar usuário..." : "Buscar caneta..."}
                    value={searchTerm}
                        ...
                        <TableRow key={index} className="hover:bg-gray-50">
                          ...
                          <TableCell>
                            <div className="flex flex-col gap-1">
                              {group.canetas.map((caneta, idx) => (
                                <div key={idx} className="flex items-center gap-2">
                                  <Badge variant="outline" className="text-xs">
                                    {caneta.marca} {caneta.modelo}
                                  </Badge>
                                  <Button
                                    variant="ghost"
                                    size="icon"
                                    className="h-6 w-6"
                                    onClick={() => handleTransferEquipment(caneta, group.usuario)}
                                  >
                                    <UserMinus className="w-3 h-3 text-orange-600" />
                                  </Button>
                                </div>
                              ))}
                              {group.canetas.length > 1 && (
                                <span className="text-xs text-pink-600 font-medium">
                                  ({group.canetas.length} canetas)
                                </span>
                              )}
                            </div>
                          </TableCell>
                ...
                <Table>
                  ...
                  <TableBody>
                    {isLoading ? (
                      ...
                      <TableRow>
                        <TableCell colSpan={7} className="text-center py-8 text-gray-500">Nenhuma caneta encontrada</TableCell>
                      </TableRow>
                    ) : (
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                <p className="text-sm text-gray-600">
                  <strong>Caneta:</strong> {equipmentToTransfer?.marca} {equipmentToTransfer?.modelo}
                </p>
                <p className="text-sm text-gray-600">
        ...
        <Dialog open={showAssignModal} onOpenChange={setShowAssignModal}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Atribuir Caneta Disponível</DialogTitle>
            </DialogHeader>
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                <p className="text-sm text-gray-600">
              ...
              <div className="space-y-2">
                <Label>Selecione a caneta disponível:</Label>
                <Select value={selectedAvailableEquipment} onValueChange={setSelectedAvailableEquipment}>
                  <SelectTrigger>
                    <SelectValue placeholder="Selecione uma caneta" />
                  </SelectTrigger>
                  <SelectContent>
                    {availableEquipments.length === 0 ? (
                      <SelectItem value="none" disabled>Nenhuma caneta disponível</SelectItem>
                    ) : (

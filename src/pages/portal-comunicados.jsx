...
function isHoje(d) { if (!d) return false; const dt = new Date(d + "T00:00:00"); return getMonth(dt) === mesAtual && getDate(dt) === diaAtual; }

function ArteStatusBadge({ artes, colaboradorId, tipo }) {
  const result = useMemo(() => {
    if (!artes || !colaboradorId || !tipo) return null;
    const pendente = artes.find(a => a.colaborador_id === colaboradorId && a.tipo_comunicado === tipo && a.ano_referencia === anoAtual && a.status_envio === "pendente");
    if (pendente) return "pronta";
    const enviada = artes.find(a => a.colaborador_id === colaboradorId && a.tipo_comunicado === tipo && a.ano_referencia === anoAtual && a.status_envio === "enviado");
    if (enviada) return "enviada";
    return "sem_arte";
  }, [artes, colaboradorId, tipo]);

  if (result === "pronta") return <Badge className="bg-green-100 text-green-800 text-xs shrink-0">✅ Arte pronta</Badge>;
  if (result === "enviada") return <Badge className="bg-gray-100 text-gray-500 text-xs shrink-0">Enviado</Badge>;
...
// ─── Formulário de Arte (igual ao admin) ──────────────────────────────────────
function ArteForm({ arte, colaboradores, currentColabName, onClose }) {
  const [tipo, setTipo] = useState(arte?.tipo_comunicado || "");
  const [colaboradorId, setColaboradorId] = useState(arte?.colaborador_id || "");
  ...
  const colabAtivos = useMemo(() => colaboradores.filter(c => c.status !== "Desligado"), [colaboradores]);
  const colabFiltrados = useMemo(() => colabAtivos.filter(c => c.nome_completo?.toLowerCase().includes(busca.toLowerCase())), [colabAtivos, busca]);
  const colabSelecionado = useMemo(() => colabAtivos.find(c => c.id === colaboradorId), [colabAtivos, colaboradorId]);
  const isGenerico = TIPOS_GENERICOS.includes(tipo);

  const createMut = useMutation({
    mutationFn: (d) => base44.entities.Comunicados_Artes.create(d),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["portal_artes"] }); onClose(); },
  });
  ...
  const handleUpload = async (e) => {
    ...
    setImagemUrl(file_url); setUploading(false);
  };
  ...
  const handleSubmit = (e) => {
    ...
    const tituloFinal = titulo || (nomeColab ? `${TIPO_LABELS[tipo]} — ${nomeColab} ${anoAtual}` : `${TIPO_LABELS[tipo]} ${anoAtual}`);
    const payload = { tipo_comunicado: tipo, colaborador_id: colaboradorId || "", colaborador_nome: nomeColab, titulo: tituloFinal, imagem_url: imagemUrl, ano_referencia: anoAtual, status_envio: arte?.status_envio || "pendente", criado_por: currentColabName || "Portal", observacoes };
    if (arte) updateMut.mutate({ id: arte.id, d: payload });
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <Select value={tipo} onValueChange={v => { setTipo(v); setColaboradorId(""); }}>
          ...
          <SelectContent>{Object.entries(TIPO_LABELS).map(([v, l]) => <SelectItem key={v} value={v}>{l}</SelectItem>)}</SelectContent>
        </Select>
      </div>
      {tipo && (
        <div>
          <Label>Colaborador {isGenerico ? "(opcional)" : "*"}</Label>
          {colabSelecionado ? (
            <div className="flex items-center gap-2 bg-indigo-50 border border-indigo-200 rounded-lg p-2 mt-1">
              <div className="flex-1"><p className="text-sm font-medium text-indigo-800">{colabSelecionado.nome_completo}</p><p className="text-xs text-indigo-600">{colabSelecionado.area}</p></div>
              <button type="button" onClick={() => setColaboradorId("")} className="text-xs text-indigo-500 hover:text-indigo-700 underline">Trocar</button>
            </div>
          ) : (
            <div className="mt-1 border rounded-lg overflow-hidden">
              <div className="p-2 border-b bg-gray-50 relative">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" />
                <input type="text" placeholder="Buscar colaborador..." value={busca} onChange={e => setBusca(e.target.value)} className="w-full pl-7 pr-2 py-1 text-sm outline-none bg-transparent" />
              </div>
              {isGenerico && <button type="button" onClick={() => { setColaboradorId(""); setBusca(""); }} className="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 text-gray-500 italic border-b">— Arte genérica</button>}
              <div className="max-h-40 overflow-y-auto">
                {colabFiltrados.slice(0, 30).map(c => (
                  <button key={c.id} type="button" onClick={() => { setColaboradorId(c.id); setBusca(""); }} className="w-full text-left px-3 py-2 hover:bg-indigo-50 border-b last:border-b-0">
      ...
      <div>
        ...
        {imagemUrl && <img src={imagemUrl} alt="preview" className="w-full max-h-40 object-contain rounded-lg border mb-2 mt-1" />}
      ...
      <div className="flex justify-end gap-3 pt-2 border-t">
        ...
        <Button type="submit" className="bg-indigo-600 hover:bg-indigo-700" disabled={createMut.isPending || updateMut.isPending}>
          {createMut.isPending || updateMut.isPending ? "Salvando..." : (arte ? "Atualizar" : "Cadastrar Arte")}
        </Button>
      </div>
    </form>
  );
...
// ─── Aba Artes ─────────────────────────────────────────────────────────────────
function AbaArtes({ colaboradores, currentColabName }) {
  ...
  const deleteMut = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["portal_artes"] }),
  });

  const filtradas = useMemo(() => {
    let r = filtroTipo === "todos" ? artes : artes.filter(a => a.tipo_comunicado === filtroTipo);
    return r.sort((a, b) => new Date(b.created_date) - new Date(a.created_date));
  }, [artes, filtroTipo]);

  const getStatusBadge = (s) => {
    if (s === "pendente") return <Badge className="bg-blue-100 text-blue-800 text-xs">⏳ Pendente</Badge>;
    if (s === "enviado") return <Badge className="bg-gray-100 text-gray-600 text-xs">✅ Enviado</Badge>;
    if (s === "erro") return <Badge className="bg-red-100 text-red-700 text-xs">❌ Erro</Badge>;
    return null;
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3 items-center justify-between">
        ...
        <Button className="bg-indigo-600 hover:bg-indigo-700 h-8 text-xs" onClick={() => { setEditando(null); setShowForm(true); }}>
      ...
      {isLoading ? (
        ...
        <div className="border rounded-lg overflow-hidden overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtradas.map(arte => (
                <tr key={arte.id} className={`border-b ${arte.status_envio === "erro" ? "bg-red-50" : arte.status_envio === "enviado" ? "bg-gray-50 opacity-70" : "hover:bg-gray-50"}`}>
                  <td className="px-4 py-3"><span className="text-xs bg-indigo-100 text-indigo-800 px-2 py-1 rounded-full font-medium">{TIPO_LABELS[arte.tipo_comunicado] || arte.tipo_comunicado}</span></td>
                  <td className="px-4 py-3">{arte.colaborador_nome ? <span className="font-medium text-gray-800">{arte.colaborador_nome}</span> : <span className="text-gray-400 italic text-xs">Arte genérica</span>}</td>
                  <td className="px-4 py-3 text-gray-600">{arte.ano_referencia || "—"}</td>
                  <td className="px-4 py-3">{arte.imagem_url ? <img src={arte.imagem_url} alt="arte" className="w-14 h-10 object-cover rounded border" /> : <span className="text-gray-400 text-xs">Sem imagem</span>}</td>
                  <td className="px-4 py-3">{getStatusBadge(arte.status_envio)}</td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      {arte.status_envio !== "enviado" && (
                        <Button size="icon" variant="ghost" onClick={() => { setEditando(arte); setShowForm(true); }}><Pencil className="w-4 h-4 text-gray-500" /></Button>
                      )}
                      <Button size="icon" variant="ghost" onClick={() => { if (confirm("Excluir esta arte?")) deleteMut.mutate(arte.id); }}><Trash2 className="w-4 h-4 text-red-500" /></Button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <Dialog open={showForm} onOpenChange={v => { if (!v) { setShowForm(false); setEditando(null); } }}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{editando ? "Editar Arte" : "Cadastrar Nova Arte"}</DialogTitle></DialogHeader>
          {showForm && <ArteForm arte={editando} colaboradores={colaboradores} currentColabName={currentColabName} onClose={() => { setShowForm(false); setEditando(null); }} />}
        </DialogContent>
      </Dialog>
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      <SecaoCard icon={Users} titulo={`Aniversariantes do Mês (${format(hoje, "MMMM", { locale: ptBR })})`} cor="text-pink-700">
        {aniversariantesMes.length === 0 ? <p className="text-sm text-gray-400">Nenhum este mês.</p> : (
          <div className="space-y-2">{aniversariantesMes.map(c => (
            <div key={c.id} className="flex items-center gap-3 bg-pink-50 border border-pink-100 rounded-lg p-3">
              {c.foto_url ? <img src={c.foto_url} alt="" className="w-10 h-10 rounded-full object-cover shrink-0" /> : <div className="w-10 h-10 rounded-full bg-pink-200 flex items-center justify-center text-pink-700 font-bold text-sm shrink-0">{c.nome_completo?.charAt(0)}</div>}
              <div className="flex-1"><p className="font-medium text-sm">{c.nome_completo}</p><p className="text-xs text-gray-500">{c.area}</p></div>
              <div className="flex items-center gap-2">
                ...
                <ArteStatusBadge artes={artes} colaboradorId={c.id} tipo="aniversario_colaborador" />
              </div>
            </div>
          ))}</div>
        )}
        ...
        {tempoEmpresa.length === 0 ? <p className="text-sm text-gray-400">Nenhum este mês.</p> : (
          <div className="space-y-2">{tempoEmpresa.map(c => (
            <div key={c.id} className="flex items-center gap-3 bg-yellow-50 border border-yellow-100 rounded-lg p-3">
              <div className="flex-1"><p className="font-medium text-sm">{c.nome_completo}</p><p className="text-xs text-gray-500">Admissão: {c.data_admissao}</p></div>
              <div className="flex items-center gap-2">
                <Badge className="bg-yellow-400 text-yellow-900 text-xs">{tipoTempoLabel(c.anos)}</Badge>
                <ArteStatusBadge artes={artes} colaboradorId={c.id} tipo="tempo_empresa_1ano" />
              </div>
            </div>
          ))}</div>
        )}
        ...
        {conjugesMes.length === 0 ? <p className="text-sm text-gray-400">Nenhum este mês.</p> : (
          <div className="space-y-2">{conjugesMes.map(c => (
            <div key={c.id} className="flex items-center gap-3 bg-red-50 border border-red-100 rounded-lg p-3">
              <div className="flex-1"><p className="font-medium text-sm">{c.nome_completo}</p><p className="text-xs text-gray-500">Cônjuge: <strong>{c.conjuge_nome || "—"}</strong></p></div>
              <div className="flex items-center gap-2">
                {isHoje(c.conjuge_data_nascimento) && <Badge className="bg-red-500 text-white text-xs">🎂 Hoje!</Badge>}
                <ArteStatusBadge artes={artes} colaboradorId={c.id} tipo="aniversario_conjuge" />
              </div>
            </div>
          ))}</div>
        )}
            ...
            return filhosAniv.map((f, i) => (
              <div key={`${c.id}-${i}`} className="flex items-center gap-3 bg-purple-50 border border-purple-100 rounded-lg p-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <ArteStatusBadge artes={artes} colaboradorId={c.id} tipo="aniversario_filho_1ano" />
                </div>
              </div>
            ));
          })}</div>
        ...
        {desligados.length === 0 ? <p className="text-sm text-gray-400">Nenhuma pendente. ✅</p> : (
          <div className="space-y-2">{desligados.map(c => (
            <div key={c.id} className="flex items-center gap-3 bg-gray-50 border border-gray-200 rounded-lg p-3">
              <div className="flex-1"><p className="font-medium text-sm">{c.nome_completo}</p><p className="text-xs text-gray-500">{c.area}</p></div>
              <div className="flex items-center gap-2">
                <ArteStatusBadge artes={artes} colaboradorId={c.id} tipo="despedida" />
                {podeEnviarDep && (
                  <Button size="sm" variant="outline" className="text-gray-700" onClick={() => enviarDespedida(c)} disabled={enviandoId === c.id + "_dep"}>
                    <CheckCircle className="w-3 h-3 mr-1" />{enviandoId === c.id + "_dep" ? "Enviando..." : "Enviar"}
                  </Button>
                )}
              </div>
            </div>

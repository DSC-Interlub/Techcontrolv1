...
async function enviarEmail(to, subject, html) {
  ...
  console.log(`[lembreteAvaliacao] to=${to} status=${response.status}`);
  return result;
}

function buildEmailAvaliacao(chamado, avaliacaoUrl) {
  const { numero_chamado, solicitante_nome, solucao, responsavel } = chamado;
...
<tr><td style="background:#fff;border-radius:0 0 10px 10px;padding:36px 40px;">
  ...
  <p style="margin:0 0 24px 0;font-size:15px;color:#6b7280;">Seu chamado foi resolvido! Leva menos de 1 minuto para avaliar o atendimento. Sua opinião é muito importante para nós.</p>
  ...
  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f8fafc;border:2px solid #e2e8f0;border-radius:10px;margin-bottom:28px;">
    ...
    <tr><td style="padding:16px 24px;font-size:14px;color:#374151;line-height:1.7;">
      ...
      ${solucao ? `<strong>Solução:</strong> ${solucao}` : ''}
    </td></tr>
  </table>

  <table width="100%" cellpadding="0" cellspacing="0" border="0">
    <tr><td align="center" style="padding-bottom:16px;">
      <a href="${avaliacaoUrl}" style="display:inline-block;background:#d97706;color:#fff;text-decoration:none;padding:14px 40px;border-radius:8px;font-size:15px;font-weight:700;">⭐ Avaliar Agora</a>
    </td></tr>
  </table>
  <p style="margin:0;font-size:12px;color:#9ca3af;text-align:center;">Use o número <strong>${numero_chamado}</strong> para acessar seu chamado na página de acompanhamento.</p>
</td></tr>
...
Deno.serve(async (req) => {
  try {
    ...
    // Busca todos os chamados aguardando avaliação (sem avaliação_data)
    ...
    console.log(`[lembreteAvaliacao] Encontrados ${aguardando.length} chamados sem avaliação`);
    ...
    for (const chamado of aguardando) {
      await enviarEmail(
        ...
        `[TechControl] Lembrete: Avalie o atendimento do chamado ${chamado.numero_chamado}`,
        buildEmailAvaliacao(chamado, avaliacaoUrl)
      );
      enviados++;
      // Intervalo para evitar rate limit do Resend
      await new Promise(resolve => setTimeout(resolve, 600));
    }

    console.log(`[lembreteAvaliacao] Enviados ${enviados} lembretes`);

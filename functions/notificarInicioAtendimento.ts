...
async function enviarEmail(to, subject, html) {
  ...
  console.log(`[notificarInicioAtendimento] to=${to} status=${response.status}`);
  return result;
}

function buildEmail(nome, responsavel, chamado) {
  const { numero_chamado, tipo_solicitacao, sistema_tipo, sistema_subtipo } = chamado;
  let tipoCompleto = tipo_solicitacao || "";
  if (tipo_solicitacao === "Sistema" && sistema_tipo) {
    tipoCompleto += ` - ${sistema_tipo}`;
    if (sistema_subtipo) tipoCompleto += ` (${sistema_subtipo})`;
  }
...
<tr><td style="background:#2563eb;border-radius:10px 10px 0 0;padding:18px 32px;text-align:center;">
...
<tr><td style="background:#ffffff;border-radius:0 0 10px 10px;padding:36px 40px;box-shadow:0 4px 24px rgba(0,0,0,0.07);">
  ...
  <p style="margin:0 0 28px 0;font-size:15px;color:#6b7280;line-height:1.6;">Seu chamado foi recebido e o atendimento já foi iniciado por <strong>${responsavel}</strong>. Estamos trabalhando para resolver sua solicitação o mais breve possível.</p>
  ...
  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f8fafc;border:2px solid #e2e8f0;border-radius:10px;margin-bottom:28px;">
    ...
    <tr><td style="padding:16px 24px;font-size:14px;color:#374151;line-height:1.7;">
      ...
      <strong>Tipo:</strong> ${tipoCompleto}
    </td></tr>
  </table>
...
Deno.serve(async (req) => {
  try {
    ...
    // Só dispara email se o status foi alterado para "Em Andamento"
    ...
    enviarEmail(
      ...
      `[TechControl] Chamado ${chamado.numero_chamado} - Em Andamento 🔧`,
      buildEmail(chamado.solicitante_nome, chamado.responsavel, chamado)
    ).catch(err => 
      console.error(`[notificarInicioAtendimento] Erro ao enviar: ${err.message}`)
    );

    console.log(`[notificarInicioAtendimento] Email disparado para ${chamado.solicitante_email}`);
    return Response.json({ success: true });
  } catch (error) {
    console.error(`[notificarInicioAtendimento] Erro: ${error.message}`);

...
async function enviarEmail(to, subject, html) {
  try {
    ...
    if (response.ok) {
      console.log(`[enviarEmailMensagemChat] Email enviado para ${to}`);
    } else {
      console.error(`[enviarEmailMensagemChat] Erro para ${to}: ${result.message}`);
    }
  } catch (error) {
...
<tr><td style="background:#ffffff;border-radius:0 0 10px 10px;padding:36px 40px;box-shadow:0 4px 24px rgba(0,0,0,0.07);">
  ...
  <p style="margin:0 0 28px 0;font-size:15px;color:#6b7280;line-height:1.6;">Você recebeu uma nova mensagem de <strong>${remetente}</strong>.</p>
  ...
  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f8fafc;border:2px solid #e2e8f0;border-radius:10px;margin-bottom:28px;">
    <tr><td style="padding:20px 24px 12px 24px;border-bottom:1px solid #e2e8f0;">
      <p style="margin:0 0 6px 0;font-size:11px;font-weight:600;color:#94a3b8;text-transform:uppercase;letter-spacing:1.5px;">Chamado #${chamado_numero}</p>
    </td></tr>
...
Deno.serve(async (req) => {
  try {
    ...
    // Buscar o chamado para obter dados do solicitante

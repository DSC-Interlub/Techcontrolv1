...
Deno.serve(async (req) => {
  try {
    ...
    // CRITICAL: Não await - apenas inicia os fetches e retorna imediatamente
    // Os emails são enviados em background pelo Resend
    ...
    }).catch(err => console.error('[portalNotificarChamado] Erro solicitante:', err.message));
    ...
    }).catch(err => console.error('[portalNotificarChamado] Erro admin:', err.message));

    // Retorna IMEDIATAMENTE sem esperar o envio dos emails

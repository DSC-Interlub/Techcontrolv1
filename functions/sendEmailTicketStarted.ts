...
Deno.serve(async (req) => {
  try {
    ...
    if (ticket.email_inicio_enviado) {
      console.log(`[sendEmailTicketStarted] Já enviado para chamado ${chamado_id}, ignorando.`);
      return Response.json({ success: true, skipped: true });
    }

    await base44.asServiceRole.entities.Chamados.update(chamado_id, { email_inicio_enviado: true });

    sendEmail(
      ticket.solicitante_email,
      `[TechControl] Chamado ${ticket.numero_chamado} — Atendimento Iniciado ⚡`,
      htmlUser(ticket, responsavel || ticket.responsavel)
    ).catch(err => console.error('[sendEmailTicketStarted] Erro:', err.message));

    return Response.json({ success: true });
  } catch (error) {
    console.error('[sendEmailTicketStarted] Erro:', error.message);

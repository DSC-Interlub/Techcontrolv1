...
async function enviarEmail(to, subject, html) {
  try {
    ...
    if (response.ok) {
      console.log(`[enviarEmailSolucao] Email enviado para ${to}`);
    } else {
      console.error(`[enviarEmailSolucao] Erro para ${to}: ${result.message}`);
    }
  } catch (error) {
    console.error(`[enviarEmailSolucao] Erro: ${error.message}`);
  }
}

function buildEmail(nome, numero, solucao) {
...
<tr><td style="background:#16a34a;border-radius:10px 10px 0 0;padding:18px 32px;text-align:center;">
  <p style="margin:0;color:#ffffff;font-size:15px;font-weight:700;letter-spacing:2px;text-transform:uppercase;">✅&nbsp;&nbsp;Solução Registrada</p>
</td></tr>
...
<tr><td style="background:#ffffff;border-radius:0 0 10px 10px;padding:36px 40px;box-shadow:0 4px 24px rgba(0,0,0,0.07);">
  ...
  <p style="margin:0 0 28px 0;font-size:15px;color:#6b7280;line-height:1.6;">A solução para seu chamado foi registrada. Confira os detalhes:</p>
  ...
  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f8fafc;border:2px solid #e2e8f0;border-radius:10px;margin-bottom:28px;">
    ...
    <tr><td style="padding:16px 24px;font-size:14px;color:#374151;line-height:1.7;">
      <strong>Solução:</strong><br>${solucao}
    </td></tr>
  </table>
...
Deno.serve(async (req) => {
  try {
    ...
    enviarEmail(
      ...
      `[TechControl] Chamado ${chamado.numero_chamado} - Solução Registrada`,
      buildEmail(chamado.solicitante_nome, chamado.numero_chamado, chamado.solucao)
    );
